...
from absa.run_base import prepare_optimizer, post_process_loss, bert_load_state_dict
...
def run_train_epoch(args, global_step, model, param_optimizer,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        # This here is totally useless, is getting annotations of spans I already have
...
def main():
    ...
    parser.add_argument("--save_proportion", default=0.5, type=float, help="Proportion of steps to save models for. E.g., 0.5 = 50% of training.")
    ...
    parser.add_argument("--max_answer_length", default=12, type=int, help="The maximum length of an answer that can be generated. This is needed because the start " "and end predictions are not conditioned on one another.")
    ...
    parser.add_argument('--loss_scale', type=float, default=128, help='Loss scaling, positive power of 2 values can improve fp16 convergence.')
    ...
    if os.path.isfile(save_path):
        ...
        logger.info("Loading model from finetuned checkpoint: '{}' (step {})".format(save_path, step))

    f = open(network_path, "w")
    for n, param in model.named_parameters():
        print("name: {}, size: {}, dtype: {}, requires_grad: {}".format(n, param.size(), param.dtype, param.requires_grad), file=f)
